...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if dm.karyawan_detail.Locate('kd_no_induk', fieldbyname('kr_no_induk')
          .Value, [loCaseInsensitive]) = true then
        begin

          salaryid := dm.karyawan_detail.fieldbyname('kd_sl_kode').Value;
          // ambil kode salary id utk mengetahui basic salary ke data salary_level
          // cari id dari salary id
          if dm.salary_level.Locate('sl_kode', salaryid, [loCaseInsensitive]) = true
          then
          begin
            ...
            begin
              sql.Text :=
                'select sum(sd_amount) as tambah from salary_level_detail ' +
            ...
            begin
              sql.Text :=
                'select sum(sd_amount) as kurang from salary_level_detail ' +
          ...
          ptid := dm.karyawan_detail.fieldbyname('kd_pt_id').Value;
          // ambil klasifikasi faktor pengurang pajak ptkp
        ...
        objekpajak := (gajiObjekpajak * 12) - ptkp;
        // penghasilan setahun diambil ketentuan penghasilan tidak kena pajak
        if objekpajak > 0 then
        begin
          potonganpajak := (objekpajak * 5 / 100) / 12;
          // 5% dikalikan objek pajak dibagi 12 utk mengetahui potongan pajak per bulan
        end
        else
        begin
          potonganpajak := 0;
        ...
        dm.inputsalary.fieldbyname('sa_kr_id').Value :=
          fieldbyname('kr_id').Value;
        ...
        dm.inputsalary.fieldbyname('sa_take_home_pay').Value := gajinetto -
          potonganpajak;
  ...
  begin
    ...
    Append;
    fieldbyname('gl_amount').Value := gajinetto * -1;
    // gaji netto sebelum diambil potongan pajak
    ...
    Append;
    fieldbyname('gl_amount').Value := gajinetto;
    // gaji netto sebelum diambil potongan pajak
    ...
    Append;
    fieldbyname('gl_amount').Value := potonganpajak;
    // gaji netto sebelum diambil potongan pajak
    ...
    fieldbyname('gl_tgl').Value := date;
    fieldbyname('gl_ref').Value := noref;
    fieldbyname('gl_desc').Value :=
      'Masukan Potongan PPh 21 untuk No Ref:' + noref;
    Post;
