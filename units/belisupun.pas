...
uses
  ...
  RpCon, RpConDS, RpBase, RpSystem, RpDefine, RpRave, ComCtrls, Vcl.DdeMan,baseformsingle;

type
  Tbelisupfrm = class(TfrmBaseSingle)
    Panel3: TPanel;
...
uses dmun, fungsi_merp, strutils, supplierun, barangviewun, custandcpun,
  calendarun, projectun,DateUtils;
{$R *.dfm}
...
begin
  ...
  begin
    ...
    params.ParamByName('spid').AsString :=
...
begin
  ...
  dt := Now;
  getMonth :=  MonthOfTheYear(dt);
  getYear := IntToStr(YearOf(dt));
  with dm.beli do
  begin
    if isBeliLangsung = 0 then
    begin
      sql.Text :=
        'SELECT * FROM beli where be_islangsung=0 ORDER BY be_id DESC LIMIT 1';
      open;
    end
    else if isBeliLangsung = 1 then
    begin
      sql.Text :=
        'SELECT * FROM beli where be_islangsung=1 ORDER BY be_id DESC LIMIT 1';
      open;
    end;

    dbpo := fieldbyname('be_kode').Value;
    getMonthdb := strToInt(AnsiMidStr(fieldbyname('be_kode').Value, 11, 2));
  ...
  BEGIN
    ...
    begin
      ...
      noPO := '001' + '/' + 'SV/PO/' + finalmonth + '/' + getYear;
      // jika tdk sama maka sistem menganggap bulan baru
    end
    else if (getMonth = getMonthdb) then
    begin
      noPO := zerofill + intToStr(getNo) + '/' + 'SV/PO/' + finalmonth + '/' +
  ...
  BEGIN
    ...
    begin
      ...
      noPO := '001' + '/' + 'SV/CA/' + finalmonth + '/' + getYear;
      // jika tdk sama maka sistem menganggap bulan baru
    end
    else if (getMonth = getMonthdb) then
    begin
      noPO := zerofill + intToStr(getNo) + '/' + 'SV/CA/' + finalmonth + '/' +
        getYear; // jika sama maka
    end;
...
begin
  ...
  if (Key = vk_return) and (griditem.SelectedField.FieldName = 'bd_sendto_nama')
  then
  begin
    ...
    if (Key = vk_return) and (griditem.SelectedField.FieldName <>
    'bd_sendto_nama') then
  begin
    aktifkanform(barangviewfrm, TBarangViewfrm);
  end;
