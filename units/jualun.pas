...
begin
  ...
  begin
    ...
    nofaktur := '010.000-' + formatdatetime('YY', date) + '.' +
      format('%8.8d', [noseri]);
    Append;
...
begin
  ...
  begin
    ...
    begin
      ...
      dm.fakturpajakdetail.fieldbyname('fd_kd_barang').Value :=
        fieldbyname('jd_kd_barang').Value;
      dm.fakturpajakdetail.fieldbyname('fd_nama_barang').Value :=
        fieldbyname('jd_nama_barang').Value;
      dm.fakturpajakdetail.fieldbyname('fd_qty').Value :=
        fieldbyname('jd_qty').Value;
      dm.fakturpajakdetail.fieldbyname('fd_harga').Value :=
        fieldbyname('jd_harga_jual').Value;
      dm.fakturpajakdetail.fieldbyname('fd_row_total').Value :=
        fieldbyname('jd_total').Value;
  ...
  begin
    ...
    BEGIN
      ...
      begin
        ...
        fieldbyname('gl_debet').Value := totalntax;
        // piutang bertambah di debet
      end
      else
      begin
    ...
    BEGIN
      ...
      if lookppn.ItemIndex = 0 then
      begin
        fieldbyname('gl_amount').Value := totalntax;
        // posting utk piutang tanpa downpayment karena downpayment dikreditkan lihat PIUTANG-2
        fieldbyname('gl_debet').Value := totalntax;
        // piutang bertambah di debet
      end
      else
      begin
        ...
        // posting utk piutang tanpa dikurangi downpayment karena besar downpayment nanti akan di kreditkan lihat di bawah
        fieldbyname('gl_debet').Value := total; // piutang bertambah di debet
      end;
    ...
    begin
      ...
      begin
        ...
        posting(downpaymentntax, 1, '110-20', strToDate(dbtgl.Text),
          notrans.Text, 'Uang Muka Penjualan Pada' + notrans.Text, 1);
        // posting kredit di akun piutang karena downpayment mengurangi total piutang
        posting(downpaymentntax, 0, '130-20', strToDate(dbtgl.Text),
          notrans.Text, 'Uang Muka Penjualan pada ' + notrans.Text, -1);
      end
      else
      begin
        // posting di akun cash
        posting(downpayment, 1, '110-20', strToDate(dbtgl.Text), notrans.Text,
          'Uang Muka Penjualan Pada' + notrans.Text, 1);
        // posting kredit di akun piutang karena downpayment mengurangi total piutang
        posting(downpayment, 0, '130-20', strToDate(dbtgl.Text), notrans.Text,
          'Uang Muka Penjualan pada ' + notrans.Text, -1);
      end;
    ...
    posting(hpp, 0, '140-10', strToDate(dbtgl.Text), notrans.Text,
      'Penyesuaian Persediaan Pada ' + notrans.Text, -1);

    if lookppn.ItemIndex = 0 then
    begin
      // posting ke hutang pajak penjualan
      posting(tax, 0, '210-80', strToDate(dbtgl.Text), notrans.Text,
        'Penjualan ' + lookcust.Text, 1);
    end;

    // posting ke akun penjualan produk
    Append;
    fieldbyname('gl_amount').Value := total;
    // posting nilai ke akun penjualan produk - tanpa nilai pajak
    ...
    Append;
    fieldbyname('gl_amount').Value := hpp;
    // posting nilai ke akun penjualan produk
...
begin
  ...
  begin
    ...
    getMonthdb := strToInt(AnsiMidStr(fieldbyname('ju_kode').Value, 11, 2));
...
begin
  ...
  if messagedlg('Simpan Transaksi ini?', mtConfirmation, [mbYes, mbNo], 0) = mrYes
  then
  begin
    hitungTotal;
    kurangiStock;
