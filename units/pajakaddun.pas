...
begin
  ...
  begin
    ...
    begin
      messagedlg('Faktur Pajak sudah ada! Mohon pilih no faktur yang lain',
        mtError, [mbOk], 0);
      abort;
    end;
  end;
  ...
  begin
    ...
    params.ParamByName('jdkd').Value := dm.tagihanview.FieldByName
      ('in_kode_jual').Value;
    open;
...
begin
  if messagedlg('Simpan Faktur Pajak ini?', mtConfirmation, [mbYes, mbNo], 0) = mrYes
  then
  begin
    simpanFaktur;
...
begin
  if messagedlg('Batalkan Faktur Pajak ini?', mtConfirmation, [mbYes, mbNo], 0)
    = mrYes then
  begin
...
procedure TpajakAddfrm.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  isPrintPajak := 0;
  Action := caFree;
end;
