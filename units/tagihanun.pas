...
begin
  ...
  begin
    ...
    fieldbyname('in_kode').Value := tempnomor +
      RightStr(dateToStr(dm.jualcari.fieldbyname('ju_tgl').Value), 7);
    post;
    applyupdates;
  end;
...
begin
  ...
  begin
    ...
    begin
      sql.Text :=
        'select * from invoice where in_kode like (:kode) order by in_id desc ';
      params.ParamByName('kode').Value := '%' + cari.Text + '%';
      open;
    end; // end of tagihanview
  end;
...
begin
  ...
  begin
    ...
    begin
      sql.Text :=
        'select * from invoice where in_cust_kode like (:kode) order by in_id desc ';
...
procedure Ttagihanfrm.DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect;
  DataCol: Integer; Column: TColumn; State: TGridDrawState);
var
  grid: TDBGrid;
  row: Integer;
...
begin
  bayarpiutang := dm.tagihanview.fieldbyname('in_amount').Value +
    dm.tagihanview.fieldbyname('in_tax').Value;
...
begin
  if dm.tagihanview.fieldbyname('in_fakturpajak').Value = 1 then
  begin
    messagedlg('Faktur Pajak untuk Invoice ini sudah di Generate!', mtWarning,
      [mbOk], 0);
    abort;
  end;
  ...
  dm.pajakinsert.fieldbyname('fp_npwp').Value :=
    dm.tagihanview.fieldbyname('custnpwp').AsString;
  dm.pajakinsert.fieldbyname('fp_nama_cust').Value :=
    dm.tagihanview.fieldbyname('customer').AsString;
  dm.pajakinsert.fieldbyname('fp_dpp').Value :=
    dm.tagihanview.fieldbyname('in_tagihan').AsString;
  dm.pajakinsert.fieldbyname('fp_ppn').Value :=
    dm.tagihanview.fieldbyname('in_tagihan_tax').Value;
  dm.pajakinsert.fieldbyname('fp_kode').Value := gNofaktur;
  dm.pajakinsert.fieldbyname('fp_cust_kode').Value :=
    dm.tagihanview.fieldbyname('in_cust_kode').Value;
  dm.pajakinsert.fieldbyname('fp_ref').Value :=
    dm.tagihanview.fieldbyname('in_kode_jual').Value;
  dm.pajakinsert.fieldbyname('fp_total_transaksi').Value :=
    dm.tagihanview.fieldbyname('in_amount').Value;
  dm.pajakinsert.fieldbyname('fp_balance').Value :=
    dm.tagihanview.fieldbyname('in_balance').Value;
